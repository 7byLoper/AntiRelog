...
public class Messages {
    ...
    public void loadValues(CustomConfig config) {
        ...
        this.pvpStoppedActionbar = config.configMessage(
                "pvp-stopped-actionbar",
                "&e&lPVP &aокончено, Вы снова можете использовать команды и выходить из игры!");
        this.inPvpBossbar =
                config.configMessage("in-pvp-bossbar", "&r&lРежим &c&lPVP &r&l- &a&l%time% &r&l%formated-sec%.");
        this.inPvpActionbar = config.configMessage(
                "in-pvp-actionbar", "&r&lРежим &c&lPVP&r&l, не выходите из игры &a&l%time% &r&l%formated-sec%.");
        this.pvpLeaved = config.configMessage(
                "pvp-leaved", "&aИгрок &c&l%player% &aпокинул игру во время &b&lПВП&a и был наказан.");
        this.commandsDisabled = config.configMessage(
                "commands-disabled",
                "&b&lВы не можете использовать команды в &e&lPvP&b&l. &b&lПодождите &a&l%time% &b&l%formated-sec%.");
        this.itemCooldown = config.configMessage(
                "item-cooldown", "&b&lВы сможете использовать этот предмет через &a&l%time% &b&l%formated-sec%.");
        this.itemDisabledInPvp = config.configMessage(
                "item-disabled-in-pvp", "&b&lВы не можете использовать этот предмет в &e&lPVP &b&lрежиме");
        this.potionCooldown = config.configMessage(
                "potion-cooldown", "&b&lВы сможете использовать это зелье через &a&l%time% &b&l%formated-sec%.");
        this.potionDisabledInPvp = config.configMessage(
                "potion-disabled-in-pvp", "&b&lВы не можете использовать это зелье в &e&lPVP &b&lрежиме");
        this.totemCooldown = config.configMessage(
                "totem-cooldown",
                "&b&lТотем небыл использован, т.к был недавно использован. Тотем будет доступен через &a&l%time% &b&l%formated-sec%.");
        this.totemDisabledInPvp = config.configMessage(
                "totem-disabled-in-pvp", "&b&lТотем небыл использован, т.к он отключен в &e&lPVP &b&lрежиме");
        this.pvpStartedWithPowerups = config.configMessage(
                "pvp-started-with-powerups",
                "&c&lВы начали пвп с включеным GM/FLY/и тд и за это получили негативный эффект");
